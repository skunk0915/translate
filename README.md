...
- **ヘッダーの動作ステータス詳細モーダル** — ヘッダーのステータスバッジ（待機中 / 音声未DL(手入力可) / データ未DL / モデル読込中 等）をタップすると、現在の接続モード（オンライン/オフライン/自動）、インターネット接続状況、現在の言語ペア、および必要な各言語モデル（音声認識モデル・各翻訳モデル）の保存・読み込み状況を一覧できるモーダルダイアログを表示。今何が使えて何が不足しているかが一目で把握でき、設定・データ管理画面へのスムーズな誘導を提供。
- **オフライン時の手入力と音声認識のモデル要件分離とオンデマンド案内** — オフライン翻訳において、手入力（テキスト入力）と音声入力（マイク）の要件を分離。容量の大きい音声認識モデル（Whisper: 約75〜250MB）が未ダウンロードであっても、各言語ペアの翻訳モデル（Marian: 約30〜60MB）さえ保存されていればキーボード手入力でのオフライン翻訳を即座に利用可能。画面を占有していた常時巨大バナーを撤廃し、音声入力時や手入力送信時に必要なデータが不足している場合のみ、その場で分かりやすくダウンロード案内（トースト＋ステータスモーダル誘導）を表示。
