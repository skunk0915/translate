...
- **ユーザー認証と履歴のサーバー永続同期（アカウント連携・アイパス式）** — ログイン認証（ID/パスワード式、初期設定: `mizy`）により、ブラウザのキャッシュ消去やプライベートブラウズ、端末変更（PC・スマホ等のマルチデバイス）を行っても過去の会話履歴が消失せず、サーバー側（SQLite: `/var/lib/transrate/history.db`）から即座に完全復元・同期。1年間有効なセッショントークン／HttpOnly Cookie によりログイン状態をシームレスに維持し、未ログイン時は認証モーダルで安全に保護します。オフライン時は端末内（IndexedDB）に自動キャッシュ・保留され、オンライン復帰時に自動で一括同期。個別削除・全削除・インライン編集も双方向に即時反映。
