...
- **翻訳結果の長押し / 原文タップ編集・再翻訳** — 会話バブルの翻訳結果（出力テキスト）を長押し（または原文タップ／編集ボタン）することで、その場でインライン編集欄を表示。誤認識の修正や、一瞬間が空いて途切れてしまった文の続きを追記して即座に再翻訳できます(履歴も自動更新)。長押し時は触覚フィードバック（バイブレーション）と押し込みアニメーションを提供。
