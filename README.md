...
- **会話の1件フォーカス表示 & 上下ナビゲーション（時系列ガイド付き）** — 会話画面（トークビュー）では、複数の吹き出しを並べず直近・選択中の1件の翻訳カードだけを画面中央に大きくフォーカス表示。翻訳した内容を相手に見せる際に周囲の他の会話が入らず一目で伝わります。上部のナビゲーションボタン（「▲ 過去」「▼ 未来」）やキーボードの上下矢印キー（↑:過去 / ↓:未来）、スマホでの上下スワイプ操作により、「上へ進めば過去へ遡り、下へ進めば未来（最新）へ進む」という自然なメンタルモデルで直感的に前後の発話へ切り替え可能。「最新」「○件前」「最古」の時系列バッジおよびカウンター（「1 / 15」）を表示し、カード本体や操作ガイドでも現在位置と移動方向を明確に伝えます。
- **履歴のスクロール一覧表示** — 履歴画面では、過去の全翻訳履歴を縦スクロールで一覧表示。過去のやり取りを一望・確認でき、各カードから個別コピー、編集・再翻訳、他言語翻訳、読み上げ、削除を快適に行えます。
...
- **文章のワンタップコピー（原文・翻訳文）** — 会話カードおよび履歴カードの原文・翻訳文テキスト本体を押すか、右横の専用コピーボタンを押すだけで、該当テキストを瞬時にクリップボードへコピー。読み上げは下部の専用ボタンで行うため、文章タップ時は誤って読み上げが鳴ることなくスマートにコピーできます。アイコンのチェックマークへの一時変化、端末の触覚フィードバック（バイブレーション）、およびトースト通知で成功をわかりやすく伝えます。多言語追加翻訳の項目にも同様のコピー機能を完備。
- **各履歴への「別の言語での翻訳」ボタンと翻訳の永続記憶** — 会話画面（トークビュー）および履歴画面の各カードに「別の言語での翻訳」ボタンを配置。任意の言語を選んでワンタップで追加翻訳を実行でき、その翻訳結果は該当履歴のデータ（`extraTranslations`）として端末（IndexedDB）およびサーバー（SQLite）へ確実に記憶・永続保存されます。再読み込みや端末切り替えを行っても追加された翻訳結果は復元・保持され、各追加言語の個別コピー・読み上げ・削除にも対応し、会話画面と履歴画面の間で双方向にリアルタイム同期されます。
