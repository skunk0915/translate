...
- **手動プッシュ・トゥ・トーク（長押し）と自動無音検知の切り替え** — 設定画面で「手動（長押し・最速）」と「自動（無音検知・ハンズフリー）」を選択可能。手動モードではマイクが事前待機（ウォームアップ）しており、ボタン（自動認識または言語ボタン）を押した瞬間（0ms）から指を離すまでの押下時間をミリ秒単位で正確に計測・録音して即座に翻訳処理を開始します。息継ぎで途中で切れてしまう誤作動が起きず、短い一言（「はい」「OK」等）も確実に拾い、話し終わりの無音待機時間（1.4秒など）がゼロになるため最速のレスポンスを実現します。自動モードでは Silero VAD による置きっぱなし会話も可能。
