...
- **手動プッシュ・トゥ・トーク（長押し）と自動無音検知の切り替え** — 設定画面で「手動（長押し・最速）」と「自動（無音検知・ハンズフリー）」を選択可能。手動モードでは、ボタン（自動認識または言語ボタン）を押している間だけ録音し、指を離した瞬間に即座に翻訳処理が始まります。息継ぎで途中で切れてしまう誤作動が起きず、話し終わりの無音待機時間（1.4秒など）がゼロになるため最速のレスポンスを実現します。自動モードでは Silero VAD による置きっぱなし会話も可能。
