...
- **会話・履歴の1件フォーカス表示 & 上下ナビゲーション（時系列ガイド付き）** — 会話画面および履歴画面では、複数の吹き出しを並べず常に直近・選択中の1件の翻訳カードだけを画面中央に大きくフォーカス表示。翻訳した内容を相手に見せる際に周囲の他の会話が入らず一目で伝わります。画面上部のナビゲーションボタン（「▲ 過去」「▼ 未来」）やキーボードの上下矢印キー（↑:過去 / ↓:未来）、スマホでの上下スワイプ操作により、「上へ進めば過去へ遡り、下へ進めば未来（最新）へ進む」というチャットアプリと同様の自然なメンタルモデルで直感的に前後の履歴へめくって切り替え可能。「最新」「○件前」「最古」の時系列バッジおよびカウンター（「1 / 15」）を表示し、カード本体や操作ガイドでも現在位置と移動方向を明確に伝えます。
